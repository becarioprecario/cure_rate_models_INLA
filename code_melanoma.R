...
d.logistic <- array(NA, dim = c(nrow(e1684), ncol(e1684) + 1, niter + 1),
  dimnames = list(NULL, c("IFN", "time", "delta", "A", "W", "z"), NULL))

d.logistic[, , 1] <- matrix(c(e1684$IFN, e1684$time, e1684$delta,
  e1684$A, e1684$W, z[, 1]), byrow = FALSE)
...
while(i <= niter) {
 ...
 logistic.inla[[i]] = inla(z ~ 1 + W + IFN + A, family = "binomial", 
  ...
  d.survival[[i]] <- array(NA, dim = c(length(which(d.logistic[,6,i] == 0)),
      ncol(e1684) + 1,niter),
    dimnames = list(NULL, c("IFN", "time", "delta", "A", "W", "z"), NULL))
  ...
  survival.inla[[i]] <- inla(inla.surv(time, delta) ~ 1 + W + IFN + A, 
  ...
  z[, i + 1] <- upz(pz[, i])
  d.logistic[, , i + 1] <- matrix(c(e1684$IFN, e1684$time, e1684$delta,
    e1684$A, e1684$W, z[, i + 1]), byrow = TRUE)
...
bma.logistic <- INLABMA:::fitmargBMA2(logistic.inla,
  rep(1 / length(nthining), length(nthining)), "marginals.fixed")
bma.survival <- INLABMA:::fitmargBMA2(survival.inla,
  rep(1 / length(nthining), length(nthining)), "marginals.fixed")
...
lapply(bma.logistic, function(X){1 - inla.pmarginal(0, X)})
...
parameters.JAGS <- c("z","eta","beta01","betaW1","betaIFN1","betaA1", "alpha","beta02","betaW2","betaIFN2","betaA2")
...
#  SUMMARY OF RESULTS
#
load("C:/Users/MICOLOGIA_JOAQUIN/Dropbox/Dropbox/cure_rate_models/paper_cure_models/code/code_melanoma.RData")
...
plot(apply(z.inla, 1, mean)[e1684$delta == 0], apply(z, 2, mean)[e1684$delta == 0],
